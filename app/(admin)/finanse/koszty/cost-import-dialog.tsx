...
// Import CSV kosztów historycznych. Krok 1: wybór pliku (parsowanie lokalne).
// Krok 2: przegląd — edytowalna kategoria RW (auto-sugestia) i stawka VAT.
// Zatwierdzenie tworzy dokumenty Cost + wpisy RwEntry (patrz commitCostImportAction).
...
/** Kategoria RW dla wiersza CSV: bezpośrednie trafienie w nazwę RW, inaczej auto-sugestia. */
...
export function CostImportDialog({ trigger }: { trigger?: ReactNode }) {
  ...
  function setRowVat(i: number, vatRate: string) {
    setRows((prev) => prev.map((r, idx) => (idx === i ? { ...r, vatRate } : r)));
  }
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "select" ? (
              <>
                Kolumny: <span className="font-medium text-foreground">Data, Dostawca, Kategoria, Kwota</span>{" "}
                (netto), opcjonalnie VAT. Można wgrać wiele miesięcy i lata 2025/2026 naraz — rok
                bierzemy z daty. Każdy wiersz trafia do Kosztów i do Rachunku wyników.
              </>
            ) : (
              <>Popraw kategorię (RW) i stawkę VAT. Kwoty to netto; koszty oznaczamy jako opłacone.</>
            )}
          </DialogDescription>
        </DialogHeader>
            ...
            {rows.length > 0 && (
              <div className="space-y-2 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  <StatusBadge tone="blue">{rows.length} {pluralPl(rows.length, "wiersz", "wiersze", "wierszy")}</StatusBadge>
                ...
                {errors.length > 0 && (
                  <div className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-300">
                    Pominięto {errors.length} {pluralPl(errors.length, "wiersz", "wiersze", "wierszy")} z błędami
                    (np. zła data/kwota): {errors.slice(0, 5).map((e) => `linia ${e.line}`).join(", ")}
                    {errors.length > 5 ? "…" : ""}
                  </div>
                )}
              </div>
